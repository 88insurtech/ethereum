...
contract 88i {
     ...
     */
    constructor (address benef, uint256 numeroApolice, uint256 valorDoBemSegurado, uint256 valorDoBem, uint256 valorDaFranquia) public {
        owner = msg.sender;
        uint256 valorSaldoApolice = valorDoBemSegurado;
        apolice = Apolice({
            beneficiario: benef,
            apoliceNumber: numeroApolice,
            valorBemSegurado: valorDoBemSegurado,
            valorBem: valorDoBem,
            valorSaldo: valorSaldoApolice,
            valorFranquia:valorDaFranquia
        });

        emit ChangeStatus(defaultStatus, valorDoBemSegurado);
    }
    ...
    /**
    * Verify if apolice is expired
    * Expiration rule: If passed 1 year without 
    **/    
    function verifyExpiration() public returns (bool)  {
        return (status == StatusApolice.VIGENTE) && (block.timestamp >= startTime + 1 years);
    }
    ...
    function liquidarContrato() public payable {
        if (verifyExpiration() && msg.sender == owner) {
            status = StatusApolice.LIQUIDADO;
            emit ChangeStatus(status, apolice.valorBemSegurado);
        }
    }
    ...
    /**
    * Retorna as informaçõe do sinistro (ID + VALOR) concatenadas 
    **/
    function getSinistrosPorId(uint256 id) public view returns (string) {
        return strConcat(
            appendUintToString("ID: ", sinistros[id].id),
            appendUintToString("VALOR: ", sinistros[id].valorSinistro)
        );
    }
    ...
    function appendUintToString(string inStr, uint _v) internal pure returns (string str) {
        uint maxlength = 100;
        bytes memory reversed = new bytes(maxlength);
        uint i = 0;
        uint v = _v;
